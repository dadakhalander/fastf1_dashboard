...
def load_resources():
    try:
        ...
        st.warning(f"Could not load data files. Running in DEMO MODE. Error: {str(e)}")
        
        np.random.seed(42)
        years = list(range(2010, 2025))
        drivers = ['Hamilton', 'Verstappen', 'Leclerc', 'Sainz', 'Alonso', 'Norris', 'Piastri', 'Bottas', 'Perez', 'Magnussen']
        teams = ['Mercedes', 'Red Bull', 'Ferrari', 'McLaren', 'Alpine', 'Williams', 'Aston Martin', 'Haas']
        
        data = []
        for year in years:
            for race in range(1, 24):
                for driver in drivers:
                    team = teams[np.random.randint(0, len(teams))]
                    data.append({
                        'raceId': race,
                        'year': year,
                        'driverRef': driver,
                        ...
                        'isWin': 1 if np.random.random() > 0.95 else 0,
                        'isPodium': 1 if np.random.random() > 0.85 else 0,
                        'points': np.random.randint(0, 26),
                        'isFinished': 1 if np.random.random() > 0.15 else 0,
                        'isDNF': 1 if np.random.random() > 0.85 else 0,
                        'raceDate': f"{year}-{race:02d}-01"
                    })
        ...
        return {
            'final_df': final_df,
            'driver_stats': final_df.groupby('driverRef').agg({'points': 'sum', 'isWin': 'sum'}),
            'constructor_stats': final_df.groupby('constructorRef').agg({'points': 'sum', 'isWin': 'sum'}),
            'scaler': None,
            'rf_winner': None,
            'gb_winner': None,
            'rf_points': None,
...
if resources is None:
    st.error("Fatal error loading resources")
    st.stop()
